...
local m_hideCheatPanel:boolean		= false;
...
function IsExpansion1Active()
	return IsRiseAndFallActive();
end

function IsExpansion2Active()
	return IsGatheringStormActive();
end
-- ===========================================================================
...
function AttachPanelToWorldTracker()
	if not isAttached then
		...
		if worldTrackerPanel then
			...
			isAttached = true;
			print("Cheat Panel attached to WorldTracker");
		else
			print("WorldTracker PanelStack not found");
		end
	end
end
...
function OnLoadGameViewStateDone()
	...
	if not isAttached then
		print("Retrying panel attachment in 1 second...");
		UI.SetTimer(function() AttachPanelToWorldTracker(); end, 1000);
	end
end
...
function OnMenuButtonToggle()
	UpdateCheatPanel(not m_hideCheatPanel);
end
