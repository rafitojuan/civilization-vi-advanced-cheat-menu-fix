...
local function RefreshPlayerContext()
	...
	if playerID ~= nil and playerID >= 0 then
		...
		if pPlayer ~= nil then
			pTreasury = pPlayer:GetTreasury();
			pReligion = pPlayer:GetReligion();
			pEnvoy = pPlayer:GetInfluence();
			pVis = PlayersVisibility[playerID];
		end
	end
end

Events.LoadGameViewStateDone.Add(RefreshPlayerContext);
